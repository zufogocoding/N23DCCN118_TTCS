...
def get_recommendations_for_user(user_id: int, limit: int = 10):
    ...
    if not user_has_vectors:
        ...
        fallback_query = """
            ...
            FROM "Song"
            WHERE "isDeleted" = false AND status = 'released'
    ...
    recommend_query = """
        ...
        FROM "Song" s, "User" u
        WHERE u.id = %s AND s."isDeleted" = false AND s.status = 'released'
          -- Exclude songs the user listened to in the last 7 days to keep it fresh
          AND s.id NOT IN (
              SELECT "songId" FROM "Interaction" 
              WHERE "userId" = %s AND "timeStamp" > NOW() - INTERVAL '7 days'
          )
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(recommend_query, (user_id, user_id, limit))
            columns = [desc[0] for desc in cur.description]
            results = []
            for row in cur.fetchall():
                results.append(dict(zip(columns, row)))
